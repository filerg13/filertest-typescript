...
describe('resource totalSupplyChart', () => {
  ...
  test.skip('retrieveWithinTimeRangeByID: only required params', async () => {
    const responsePromise = client.coins.totalSupplyChart.retrieveWithinTimeRangeByID('bitcoin', {
      from: 0,
      to: 0,
    });
    const rawResponse = await responsePromise.asResponse();
  ...
  test.skip('retrieveWithinTimeRangeByID: required and optional params', async () => {
    const response = await client.coins.totalSupplyChart.retrieveWithinTimeRangeByID('bitcoin', {
      from: 0,
      to: 0,
    });
  });
