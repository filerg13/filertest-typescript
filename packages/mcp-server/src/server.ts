...
import { ClientOptions } from 'filertest-typescript';
...
export { ClientOptions } from 'filertest-typescript';
export { endpoints } from './tools';

// Create server instance
export const server = new McpServer(
  {
    name: 'filertest_typescript_api',
...
export function initMcpServer(params: {
  ...
  const client = new Filertest(params.clientOptions);
  const capabilities = {
    ...defaultClientCapabilities,
...
export function init(params: {
  ...
  const client =
    params.client || new Filertest({ environment: (readEnv('FILERTEST_ENVIRONMENT') || undefined) as any });
